...
config = load_config('1/config.json')
...
def get_random_word(session, tguser_id):
    ...
    q = sq.select(Words.word, Words.translate).join(TgUserWord, Words.id == TgUserWord.word_id).join(Users,
                                                                                                     TgUserWord.user_id == Users.id) \
...
def get_other_words(session, tguser_id, except_word):
    ...
    q = sq.select(Words.translate).join(TgUserWord, Words.id == TgUserWord.word_id).join(Users,
                                                                                         TgUserWord.user_id == Users.id) \
...
def delete_word(session, tguser_id, word):
    ...
    session.query(TgUserWord).filter(
        TgUserWord.user_id == session.query(Users).filter_by(tguser_id=tguser_id).first().id). \
