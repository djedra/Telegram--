...
config = tgdb.load_config('1/config.json')
DSN = tgdb.dsn(config)
engine = tgdb.sq.create_engine(DSN)
tgdb.create_db(engine)
Session = tgdb.sessionmaker(bind=engine)
session = Session()

tgdb.import_words(session)
...
known_users = tgdb.get_users(session)
userStep = {}
buttons = []
...
def create_cards(message, step=0):
    ...
    global buttons
    buttons = []
    word_tuple = tgdb.get_random_word(session, cid)
    if not word_tuple:
    ...
    others = tgdb.get_other_words(session, cid, target_word)
...
def delete_word(message):
    ...
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        tgdb.delete_word(session, message.from_user.id, data['translate_word'])
...
def get_ru_word(message):
    ...
    if message.text == Command.CANCEL:
        ...
        bot.register_next_step_handler(message, get_ru_word)
    elif tgdb.is_cyrillic(message.text):
        target_word = tgdb.translate_word(message.text, token=tgdb.ya_token)
        markup = types.ReplyKeyboardMarkup(row_width=2)
...
def get_en_word(message, word, target_word):
    ...
    if message.text == Command.CANCEL:
        ...
        elif tgdb.is_english(message.text):
...
def add_word_to_db(tid, word, translate):
    ...
    resp = tgdb.add_new_word(session, tid, word, translate)
    if resp == 1:
